...
def _convert_oci_tool_call_to_langchain(tool_call: Any) -> ToolCall:
    ...
    _id = uuid.uuid4().hex[:]
    return ToolCall(name=tool_call.name, args=json.loads(tool_call.arguments) if hasattr(tool_call,"arguments") else tool_call.parameters, id=_id)
...
class MetaProvider(Provider):
    ...
    def __init__(self) -> None:
        ...
        self.oci_chat_function_definition = models.FunctionDefinition
        self.oci_chat_function_call = models.FunctionCall

    def chat_response_to_text(self, response: Any) -> str:
        if response.data.chat_response.choices[0].message.content:
    ...
    def convert_to_oci_tool(
        ...
        if not tool:
            return None
        return self.oci_chat_function_definition(type="FUNCTION", **tool)
...
class ChatOCIGenAI(BaseChatModel, OCIGenAIBase):
    ...
    def _generate(
        ...
        **kwargs: Any,
        ...
        if "tool_calls" in generation_info or generation_info["finish_reason"]=="tool_calls":
            tool_calls = [
                _convert_oci_tool_call_to_langchain(tool_call)
                for tool_call in response.data.chat_response.choices[0].message.tool_calls
            ]
        else:
            tool_calls = []
